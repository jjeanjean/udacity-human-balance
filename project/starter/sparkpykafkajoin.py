...
spark=SparkSession.builder.appName('stedis-join').getOrCreate()
...
zSetEntriesDecodedStreamingDF.withColumn('encodedCustomer', from_json('encodedCustomer',customerMessageSchema)).select(col('encodedCustomer.*')).createOrReplaceTempView('CustomerRecords')
...
emailAndBirthDayStreamingDF=spark.sql('select customerName, email, phone, birthDay from CustomerRecords where birthDay is not null')
...
# Be sure to specify the option that reads all the events from the topic including those that were published before you started the spark stream
stedisEventsRawStreamingDF = spark.readStream.format('kafka').option('kafka.bootstrap.servers','localhost:9092').option('subscribe','stedi-events').option('startingOffsets','earliest').load()

# TO-DO: cast the value column in the streaming dataframe as a STRING 
stedisEventsStreamingDF=stedisEventsRawStreamingDF.selectExpr('cast(key as string) key', 'cast(value as string) value')
...
stedisEventsStreamingDF.withColumn('value', from_json('value',customerRiskMessageSchema)).select(col('value.*')).createOrReplaceTempView('CustomerRisk')
...
customerInformationDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("""email=customer"""))
...
customerInformationDF.selectExpr('cast(customer as string) key', 'to_json(struct(*)) as value').writeStream.format('kafka').option('kafka.bootstrap.servers', 'localhost:9092').option('topic', 'risk-score').option('checkpointLocation', '/tmp/kafkacheckpoint').start().awaitTermination()
